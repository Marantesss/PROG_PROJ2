...
bool Board::checkFinalBoard() // Checks if everything is correct in the board before saving it
{
	string word;
	// Checks all horizontal words
	for (int i = 0; i < numLines; i++) {
		word.clear();
		for (int j = 0; j < numColumns; j++) {
			if (isalpha(board.at(i).at(j)))
				word += board.at(i).at(j);
			if ((board.at(i).at(j) == '#' && !word.empty()) || (j == (numColumns - 1) && !word.empty()) && word.size() != 1) // If end of word is reached
				for (int k = 0; k < position_words.size(); k++)
					if (position_words.at(k).second == word) { // Word matches with the ones inserted
						word.clear(); // Cleans the string
						break; // Stops cicle (word is valid)
					}
					else if (k == position_words.size() - 1) // Word was not found
						return false;
			if (board.at(i).at(j) == '.')
				word.clear();
		}
	...
	// Checks all vertical words
	for (int i = 0; i < numColumns; i++) {
		word.clear();
		for (int j = 0; j < numLines; j++) {
			if (isalpha(board.at(j).at(i)))
				word += board.at(j).at(i);
			if ((board.at(j).at(i) == '#' && !word.empty()) || (j == (numLines - 1) && !word.empty()) && word.size() != 1) // If end of word is reached
				for (int k = 0; k < position_words.size(); k++)
					if (position_words.at(k).second == word) { // Word matches with the ones inserted
						word.clear(); // Cleans the string
						break; // Stops cicle (word is valid)
...
void Board::resetBoard() // Resets the board to empty, and then adds the words in vector position_words
{
	for (int i = 0; i < numLines; i++) // Fills the vector board with "."
		for (int j = 0; j < numColumns; j++)
			board.at(i).at(j) = '.';
